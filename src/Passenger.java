...
public class Passenger {
    ...
    public boolean isContainsName(){
        ...
        if(ManageScreen.name.length() != Constants.INITIALIZE){
            for (int i = 0; i < ManageScreen.name.length(); i++) {
                if(Character.isAlphabetic(ManageScreen.name.charAt(i))){
                    isValid = true;
                }else {
                    isValid = false;
                    break;
            ...
            if(isValid){
                if(this.getName().contains(ManageScreen.name)){
                    contains = true;
                }
            }
        }
    ...
    public boolean isEqualsSibSp(){
        return this.sibSp == ManageScreen.sibSp;
    }
    public boolean isEqualsParch(){
        return this.parch == ManageScreen.parch;
    }
    public boolean isContainsTicket(){
        return this.ticket.contains(ManageScreen.ticket);
    }
    public boolean isAboveMinFare(){
        boolean aboveMin = false;
        double minFare = ManageScreen.minFare;
    ...
    public boolean isBelowMaxFare(){
        ...
        double maxFare = ManageScreen.maxFare;
    ...
    public boolean isContainsCabin(){
        boolean contains = false;
        if(ManageScreen.cabin.length() != Constants.INITIALIZE){
            if(this.getCabin().contains(ManageScreen.cabin)){
                contains = true;
            }
        }
        return contains;
